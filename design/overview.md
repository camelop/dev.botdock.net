...
- **Account credit tracker**：一个专门的页面，统一展示各类账号的 credit 余额 / 用量（例如 Anthropic API、Claude Code 订阅、OpenAI、Cloudflare 等）。支持多账号，按账号配置拉取 credit 信息的方法（可能用 secret 保存的 API key + 一段拉取脚本 / 内置 adapter），并在 Dashboard 给出"哪个账号快用光了"的提示。
