...
- **M0** 存储层（目录规范、TOML/JSON/NDJSON 读写）+ keys/machines/secrets CRUD（CLI 先行）
- **M1** React SPA + REST/WS，覆盖 M0
- **M2** `generic-cmd` session：远端启动 + tmux + 日志 tail
- **M3** `claude-code` session：transcript 规范化 + running/waiting 状态识别 + 任务下发
- **M4** Resource 推送（镜像到 `.botdock/`）+ 配套 CC skill
- **M5** git-repo 资源（含 deploy key 注入）+ 多跳 jump host 打磨

（舍弃 / 延后：session 复现、日志轮转、多用户、加密）

### 未来想做的

- **Account credit tracker**：一个专门的页面，统一展示各类账号的 credit 余额 / 用量（例如 Anthropic API、Claude Code 订阅、OpenAI、Cloudflare 等）。支持多账号，按账号配置拉取 credit 信息的方法（可能用 secret 保存的 API key + 一段拉取脚本 / 内置 adapter），并在 Dashboard 给出"哪个账号快用光了"的提示。**v0.0.13 已落地手动管理版本**；自动拉取各 provider 余额留给未来的专用 agent。
- **ttyd 终端嵌入**（取代 claude-code 的 Live log）：远端跑 `ttyd tmux attach -t <tmux_session>`，BotDock 动态分配一条 local SSH port-forward 接到 ttyd 端口，再用 web server 反代 HTTP + WS 到 `/api/sessions/:id/terminal/*`。UI 上 claude-code 的 Live log 区位变成 "Open terminal" 按钮或直接 embed 一个 iframe，用户体验就是原生终端——cursor 定位 / TUI / 颜色全部交给真正的终端模拟器，不再由我们自己画。**Provisioning 变化**：session 启动时顺带 spawn ttyd、分配 forward，把 `terminal_url` 写进 session meta。**前置**：远端需要预装 ttyd（给清晰安装提示），BotDock 需要加一个通用 HTTP+WS 反向代理模块（同 path 前缀路由到 `localhost:<forward_port>`）。
